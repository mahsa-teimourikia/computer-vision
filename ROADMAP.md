...
6. [**Multimodal Adaptation & Continual Learning: From Domain Shift to Safe Capability Evolution**](curriculum/advanced/06-multimodal-adaptation-continual-learning/README.md) — shift characterization, frozen-feature baselines, projectors, adapters, prompts, LoRA, partial/full fine-tuning, multimodal alignment drift, replay, EWC, distillation, parameter isolation, forgetting matrices, capability regression gates, lineage, promotion, and rollback. *(available)*
7. [**Robustness, Uncertainty & Failure Recovery: From Distribution Shift to Risk-Aware Vision Systems**](curriculum/advanced/07-robustness-uncertainty-failure-recovery/README.md) — task-specific corruption curves, ambiguity and model disagreement, calibration under shift, OOD versus error detection, conformal prediction sets, risk–coverage, cost-aware abstention, fail-closed evidence gates, and independently verified bounded recovery. *(available)*
8. [**Efficient Spatial & Multimodal Inference: From Profiling to Edge-Aware Deployment**](curriculum/advanced/08-efficient-spatial-multimodal-inference/README.md) — workload contracts, end-to-end profiling, resolution/token budgets, quantization, pruning, multimodal distillation, export parity, batching, queueing, caching, temporal reuse, Pareto analysis, and capability-preserving edge decisions. *(available)*
9. [**Production Spatial AI Operations & Observability: From Deployment Contracts to Incident Recovery**](curriculum/advanced/09-production-spatial-ai-operations-observability/README.md) — immutable deployment manifests, typed registries, time-valid frame graphs, calibration health, capability SLOs, delayed outcomes, drift taxonomy, shadow/canary gates, kill switches, stateful rollback, verified recovery, incident evidence, and audit. *(available)*
