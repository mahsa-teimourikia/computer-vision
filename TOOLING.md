...
Advanced 08 implements end-to-end host profiling, resolution and token budgets, INT8-like calibration experiments, structured width pruning, task-only versus multimodal distillation, `torch.export` parity, batching and saturation sweeps, bounded-queue simulation, authorization-aware cache keys, temporal reuse, Pareto analysis, frozen gates, and digest-bound evidence directly with PyTorch, NumPy, pandas, SciPy, scikit-learn, and Matplotlib. torchao, ExecuTorch, ONNX Runtime, TensorRT, Triton, OpenVINO/NNCF, Core ML Tools, and LiteRT remain disabled, revision-governed target mappings. Review target hardware, operator coverage, dynamic shapes, calibration representativeness, numeric parity, cold start, tail latency, queue policy, cache scope and invalidation, model/processor lineage, capability regressions, and rollback readiness before use. FLOPs, MACs, sparsity, artifact size, and notebook timings are evidence—not substitutes for a matched benchmark on the deployment target.
