...
Advanced 07 implements corruption/severity evaluation, reliability diagrams, ECE/Brier/NLL, temperature scaling, deep-ensemble and MC-dropout evidence, centroid and regularized Mahalanobis distance, energy scores, AUROC/AUPR/FPR@95, split-conformal prediction sets, risk–coverage, fail-closed policy, and bounded recovery directly with PyTorch, NumPy, pandas, SciPy, scikit-learn, and Matplotlib. TorchUncertainty, OpenOOD, Alibi Detect, RobustBench, and MAPIE remain disabled, revision-governed optional integrations. Review score orientation, calibration and threshold populations, conformal assumptions, corruption protocol, benchmark license/checkpoint lineage, dependency health, recovery limits, and independent post-recovery verification before use.
