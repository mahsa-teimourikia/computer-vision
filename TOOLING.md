...
Advanced 07 implements corruption/severity evaluation, reliability diagrams, ECE/Brier/NLL, temperature scaling, deep-ensemble and MC-dropout evidence, explicit metric/score orientation, centroid and regularized Mahalanobis distance, energy scores, AUROC/AUPR/FPR@95, split-conformal prediction sets, empty-set-safe risk–coverage, fail-closed policy, and two-stage bounded recovery directly with PyTorch, NumPy, pandas, SciPy, scikit-learn, and Matplotlib. TorchUncertainty, OpenOOD, Alibi Detect, RobustBench, and MAPIE remain disabled, revision-governed optional integrations. Review score orientation, calibration and threshold populations, minimum coverage, conformal assumptions, corruption protocol, benchmark license/checkpoint lineage, dependency health, recovery limits, and independent post-recovery verification before use.

Advanced 08 implements repeated-region host profiling with dispersion, deterministic service/load simulation for stable teaching gates, resolution and token budgets, INT8-like calibration experiments, structured width pruning, task-only versus multimodal distillation, numeric and behavioral `torch.export` parity, batching and saturation sweeps, bounded-queue simulation, principal-aware cache keys, temporal reuse, global-versus-feasible Pareto analysis, explicit keep-reference semantics, frozen gates, and digest-bound evidence directly with PyTorch, NumPy, pandas, SciPy, scikit-learn, and Matplotlib. torchao, ExecuTorch, ONNX Runtime, TensorRT, Triton, OpenVINO/NNCF, Core ML Tools, and LiteRT remain disabled, revision-governed target mappings. Review target hardware, operator coverage, dynamic shapes, calibration representativeness, numeric and behavioral parity, cold start, tail latency, queue policy, cache scope and invalidation, model/processor lineage, capability regressions, and rollback readiness before use. FLOPs, MACs, sparsity, artifact size, simulation outputs, and notebook timings are evidence—not substitutes for a matched benchmark on the deployment target.

Advanced 09 implements immutable deployment/configuration identity, registry references, deterministic compatibility checks, observation-time frame resolution, cycle closure, persistence-aware calibration health, capability dependency impact, bounded-cardinality telemetry, explicit drift families, delayed-outcome joins, sample-aware SLO gates, proxy-aware canary holds, stateful rollback compatibility, selective kill switches, incident timelines, independent recovery verification, and audit evidence directly with NumPy, pandas, SciPy, and Matplotlib. MLflow, ROS 2 tf2/rosbag2, OpenTelemetry, Prometheus/Grafana, OpenLineage, SLSA/Sigstore, OpenFeature, Evidently/NannyML, and Argo Rollouts/Flagger remain disabled production mappings. Review identity, validity intervals, clock semantics, registry authority, telemetry schemas/cardinality, label coverage and bias, state migration, cache invalidation, control-plane access, evidence retention, disaster recovery, and post-recovery capability probes before use. A registry entry, dashboard, drift signal, or successful rollback command is evidence—not proof that a capability is correct or recovered.
...
| [ExecuTorch](https://docs.pytorch.org/executorch/) | PyTorch export to mobile and embedded targets | Edge-oriented export, quantization, delegates, and runtime tooling | Backend coverage, partitioning, quantizer/backend coupling, memory planning, and device validation |
| [torchao](https://docs.pytorch.org/ao/stable/) | PyTorch-native quantization and sparsity experiments | Composable low-precision and sparsity primitives | Kernel/backend support, calibration, accuracy drift, and evolving API compatibility |
